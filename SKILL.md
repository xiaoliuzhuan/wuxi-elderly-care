...
This repository also ships a Codex plugin package at `.codex-plugin/plugin.json`, a bundled MCP config at `.mcp.json`, and a repo marketplace entry at `.agents/plugins/marketplace.json` so the same codebase can be distributed as a plugin-shaped package instead of only a raw skill folder.
